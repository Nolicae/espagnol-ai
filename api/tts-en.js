// Vercel serverless function — nolicae-tts consolidated space (English voices)
...
async function englishTTS(text, voice = DEFAULT_VOICE) {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS.length; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        lastErr = new Error(`english-tts ${response.status}`);
        continue;
      }
...
module.exports = async function handler(req, res) {
  ...
  try {
    const audio = await englishTTS(text, voice);
